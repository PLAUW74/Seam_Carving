...
def find_vertical_seam_greedy(energy_map):
    ...
    # Find the start of the seam in the first row
    ...
    for i in range(1, height):
        ...
        if current_col == 0:
            ...
            min_index = np.argmin(neighborhood)
            current_col = min_index

        # Handle right boundary
        elif current_col == width - 1:
            neighborhood = energy_map[i, width - 2:width]  # col w-2, w-1
            min_index = np.argmin(neighborhood)
            current_col = (width - 2) + min_index

        # General case
        else:
            neighborhood = energy_map[i, current_col - 1:current_col + 2]
            min_index = np.argmin(neighborhood)
            current_col = (current_col - 1) + min_index
...
def carve(image, num_seams, direction):
    ...
    for k in range(num_seams):
        ...
        # Find seam using greedy algorithm
...
def main():
    parser = argparse.ArgumentParser(
        description='Greedy Algorithm Seam Carving'
    )
    parser.add_argument(
