...
def create_comparison_image(input_path, output_path, layout='vertical'):
    """
    Creates a comparison image showing input above and output below (or side-by-side).
    ...
    input_labeled = add_label(input_img.copy(), f"Original ({input_w}x{input_h})")
    output_labeled = add_label(output_img.copy(), f"Carved ({output_w}x{output_h})")
    ...
    if layout == 'vertical':
        ...
        if input_w != max_width:
            ...
            input_labeled = cv2.resize(input_labeled, (max_width, int(input_h * scale)))
        
        if output_w != max_width:
            scale = max_width / output_w
            output_labeled = cv2.resize(output_labeled, (max_width, int(output_h * scale)))
        ...
        if input_h != max_height:
            ...
            input_labeled = cv2.resize(input_labeled, (int(input_w * scale), max_height))
        
        if output_h != max_height:
            scale = max_height / output_h
            output_labeled = cv2.resize(output_labeled, (int(output_w * scale), max_height))
...
def main():
    ...
    if args.display:
        ...
        cv2.imshow('Image Comparison (Press any key to close)', comparison_display)
        cv2.waitKey(0)
